// 📜 Gestion des animés - VERSION CORRIGÉE v2
...
async function fetchAnimes({ query = "", page = 1, year = "", category = "" } = {}) {
    try {
        const params = new URLSearchParams({ api_key: window.API_KEY, page });
        let url;
        ...
        if (query.trim()) {
            ...
            params.append("with_genres", category || "16");
        ...
        const filteredResults = query.trim()
            ? data.results
            : data.results.filter(anime => anime.origin_country && anime.origin_country.includes("JP"));

        if (page === 1) {
            window.allAnimes = filteredResults;
        } else {
            window.allAnimes = [...window.allAnimes, ...filteredResults];
        }
...
function displayAnimes() {
    ...
    const animesToDisplay = window.showFavoritesOnly
        ? window.allAnimes.filter(anime => window.favorites && window.favorites.has(String(anime.id)))
        : window.allAnimes;
        ...
        <div class="card-container">
            ...
            <div class="card-buttons">
                <img class="favorite-icon"
                     src="${isFavorite ? "./assets/img/check.svg" : "./assets/img/heart-filled.svg"}"
                     data-id="${id}"
...
async function openModal(animeId) {
    ...
    if (animeDetails) {
        ...
        if (elements.date)
            elements.date.textContent = `Première diffusion : ${animeDetails.first_air_date || "Inconnue"}`;
        if (elements.rating)
            elements.rating.textContent = `Note : ${animeDetails.vote_average}/10`;
        if (elements.genres) {
        ...
        if (elements.description)
            elements.description.textContent = "Impossible de charger les détails de l'anime.";
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Seulement sur la page principale (pas sur favoris.html)
